function getCookies() {
    ...
    if (!cookieStore) {
        document.getElementById("cookieStore").innerHTML = "Cookie Store API not supported";
        return;
    }
    ...
    cookieStore.getAll().then(cookies => {
        ...
        document.getElementById("cookieStore").innerHTML = JSON.stringify(cookies, null, 2);
    });
  }
  ...
  function setCookiesFromClient() {
    ...
    let cookie = `${encodeURIComponent(key)}=${encodeURIComponent(cookieValue)};max-age=${30*24*60*60}`;
